...
class ConsoleTest {
    ...
    static void start() {
        ...
        apiService = new APIService(urlBuilderFactory, processorFactory, apiConnection, objectChecker, jsonConverter, parameterChecker)
    }
    ...
    void testCorrectArguments() {
        assertEquals("[latitude:48.000, longitude:131.000, count:3]", apiService.checkInput(["-p", "48.000;131.000;3"]).toString())
        assertEquals("[latitude:90, longitude:180, count:1]", apiService.checkInput(["-p", "90;180;1"]).toString())
        assertEquals("[latitude:49.1, longitude:50.6, count:11]", apiService.checkInput(["-p", "49.1;50.6;11"]).toString())
        assertEquals("[latitude:45.438, longitude:46.538, count:25]", apiService.checkInput(["-p", "45.438;46.538;25"]).toString())
        assertEquals("[latitude:40.438342, longitude:41.538, count:40]", apiService.checkInput(["-p", "40.438342;41.538;40"]).toString())
    }
    ...
    void testSearch() {
        ...
        list.each {
            def result = apiService.search(apiService.checkInput(it))
